...
class _DoctorDashboardScreenState extends State<DoctorDashboardScreen> {
  ...
  int _missedAppointmentsCount = 0;
  ...
  void _updateNetworkState(dynamic result) {
    ...
    log('Network state updated: disconnected=$isDisconnected, raw_result=$result', name: 'DoctorDashboard');
  ...
  void _loadUser() async {
    ...
    if (user?.backendId != null) {
      ...
      ChatService.initialize(user.backendId!).then((_) {
        // Subscribe to real-time messages to keep chat list updated + play sound
      ...
      if (mounted) {
        ...
        for (var appt in appointments) {
          ...
          if (apptDay.isAtSameMomentAs(today)) {
            todayCount++;
          } else if (apptDay.isBefore(today)) {
            missedCount++;
          }
        }
        setState(() {
          _todayAppointmentsCount = todayCount;
          _missedAppointmentsCount = missedCount;
        });
      }
    } catch (e) {
  ...
  Future<void> _fetchUnreadNotifCount(int userId) async {
    try {
      ...
      log('Could not load unread notification count: $e', name: 'DoctorDashboard');
    }
  }
      ...
      bottomNavigationBar: CustomBottomNavBar(
        ...
        items: [
          ...
          CustomNavItem(
            ...
            badgeCount: _chatContacts.fold<int>(0, (sum, c) => sum + c.unreadCount),
          ),
          CustomNavItem(
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        AnimatedListItem(
                          ...
                          child: StatCard(
                            ...
                            label: AppLocalizations.of(context)?.get('todayAppointments') ?? 'Today\'s Appointments',
                            color: Colors.purple,
                            onTap: () async {
                              await Navigator.pushNamed(
                                context,
                                '/doctor_appointments',
                                arguments: {'todayOnly': true, 'initialIndex': 0},
                              );
                              if (_currentUser?.backendId != null) {
                                _fetchAppointments(_currentUser!.backendId!);
                              }
                    ...
                    AnimatedListItem(
                      ...
                      child: StatCard(
                        ...
                        value: '$_missedAppointmentsCount',
                        label: AppLocalizations.of(context)?.get('missedAppointments') ?? 'المواعيد الفائتة',
                        color: AppColors.error,
                        onTap: () async {
                          await Navigator.pushNamed(
                            context,
                            '/doctor_appointments',
                            arguments: {'initialIndex': 1}, // Missed tab will be index 1
                          );
                          if (_currentUser?.backendId != null) {
                            _fetchAppointments(_currentUser!.backendId!);
                          }
                  ...
                  : _patientsError != null
                      ...
                      : _filteredPatients.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemCount: _filteredPatients.length + 1,
                              itemBuilder: (context, index) {
                                // Extra item at the end for FAB spacing
